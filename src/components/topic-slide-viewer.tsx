...
export function TopicSlideViewer({
  slide,
  topicTitle,
  open,
  onOpenChange,
}: TopicSlideViewerProps) {
  ...
  const frameUrl = signedUrl && slide && isPowerPoint(slide)
      ...
      <DialogContent className="h-[min(92vh,900px)] max-h-[92vh] w-[min(96vw,1400px)] max-w-none gap-3 overflow-hidden border-0 bg-slate-950 p-3 text-white shadow-2xl sm:rounded-xl">
        <DialogHeader className="flex-row items-center justify-between pr-10 text-left">
          <div className="min-w-0">
            ...
            <DialogDescription className="truncate text-slate-400">{slide?.file_name}</DialogDescription>
          </div>
          {signedUrl ? (
            <div className="flex shrink-0 gap-1">
              <Button asChild size="icon" variant="ghost" className="text-slate-300 hover:bg-white/10 hover:text-white">
                <a href={signedUrl} download={slide?.file_name} aria-label="Download slides">
                  <Download />
                </a>
              </Button>
              <Button asChild size="icon" variant="ghost" className="text-slate-300 hover:bg-white/10 hover:text-white">
                <a href={signedUrl} target="_blank" rel="noreferrer" aria-label="Open slides in a new tab">
                  <ExternalLink />
                </a>
              </Button>
            </div>
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center text-slate-400">
              ...
              {signedUrl ? (
                <Button asChild variant="secondary">
                  <a href={signedUrl} download={slide?.file_name}>Download file</a>
                </Button>
              ) : null}
            </div>
          )}
